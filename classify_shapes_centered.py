...
def main():
    ...
    model = generate_linear_classifier(1000)#Train model on 1000 samples
    [X_test, Y_test] = generate_test_set_classification(100)#Test model on 100 shapes
    ...
    for k in range(len(X_test)):
        pred = model.predict(X_test[k].reshape(1, X_test[k].shape[0]))
